...
public class PhoneCamera : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            InvokeRepeating("TFClassify", 3.0f, 3.0f);
        }
    }
            ...
            StartCoroutine(this.classifier.Classify(result, probabilities =>
            {
                ...
                if (probabilities.Any())
                {
                    ...
                    for (int i = 0; i < 3; i++)
                    {
                        ...
                        found.Add(probabilities[i].Key);                        
                    }

                    if(found.Any(item => item == "ashcan, trash can, garbage can, wastebin, ash bin, ash-bin, ashbin, dustbin, trash barrel, trash bin" ||
                                         item == "computer keyboard, keypad"))
                    {
